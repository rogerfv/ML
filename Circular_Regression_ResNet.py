...
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '5, 6, 7'  # Selecciona les GPUs 5, 6 i 7
...
# Dataset y loaders
full_dataset = TensorDataset(X_aug_rgb, Y_aug_encoded)
train_ds = Subset(full_dataset, train_idx)
test_ds = Subset(full_dataset, test_idx)

train_loader_2 = DataLoader(train_ds, batch_size=32, shuffle=True, num_workers=4, pin_memory=True)
test_loader_2 = DataLoader(test_ds, batch_size=32, num_workers=4, pin_memory=True)
...
class ResNetForRegression(nn.Module):
    def __init__(self):
        ...
        self.base.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 2)  # regresión: salida escalar
        )
...
# Initialize the run
wandb.init(project="cnn-multigpu-resnet-circular", name="run_01", config={
    "epochs": 100,
    "batch_size": 32,
    "learning_rate": 1e-3,
})
...
def circular_loss(preds, targets):
    # preds y targets: shape [batch, 2] → [cosθ, sinθ]
...
torch.save(model_2.state_dict(), "angle_cnn_weights_res_net_circular.pth")
end = time.time()
...
with torch.no_grad():
    for images, targets in test_loader_2:
        ...
        preds = model_2(images)  # shape [B, 2]
        all_preds.append(decode_angles(preds))
        # targets: [B, 2] → queremos el ángulo real
